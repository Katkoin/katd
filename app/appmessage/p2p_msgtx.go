...
// Outpoint defines a katkoin data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new katkoin transaction outpoint point with the
...
// TxIn defines a katkoin transaction input.
type TxIn struct {
...
// NewTxIn returns a new katkoin transaction input with the provided
...
// TxOut defines a katkoin transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new katkoin transaction output with the provided
...
// MsgTx implements the Message interface and represents a katkoin tx message.
