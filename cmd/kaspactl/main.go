...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		katdMessage, err := client.Post(&protowire.katdMessage{Payload: &protowire.katdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := katdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	katdMessage := &protowire.katdMessage{}
	err := protojson.Unmarshal([]byte(response), katdMessage)
	if err != nil {
	...
	return marshalOptions.Format(katdMessage)
}
