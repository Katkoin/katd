...
// 2. A wrapper that implements iskatdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `katdMessage_` prefix and `Request` suffix
...
// generatekatdMessage generates a wrapped katdMessage with the given `commandValue`
func generateKatdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.KatdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	katdMessage := reflect.New(reflect.TypeOf(protowire.KatdMessage{}))
	katdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return katdMessage.Interface().(*protowire.KatdMessage), nil
}
