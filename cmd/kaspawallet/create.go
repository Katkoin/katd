...
import (
	...
	"github.com/katkoin/katdd/cmd/katwallet/libkatwallet/bip32"
...
func create(conf *createConfig) error {
	...
	fmt.Printf("Notice the above is neither a secret key to your wallet " +
		"(use \"katwallet dump-unencrypted-data\" to see a secret seed phrase) " +
		"nor a wallet public address (use \"katwallet new-address\" to create and see one)\n\n")
	...
	if len(signerExtendedPublicKeys) > 0 {
		cosignerIndex, err = libkatwallet.MinimumCosignerIndex(signerExtendedPublicKeys, extendedPublicKeys)
		if err != nil {
			return err
		}
	}
