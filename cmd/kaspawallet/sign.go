...
func sign(conf *signConfig) error {
	...
	for i, partiallySignedTransaction := range partiallySignedTransactions {
		updatedPartiallySignedTransactions[i], err =
			libkatwallet.Sign(conf.NetParams(), privateKeys, partiallySignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
	}
	...
	for _, updatedPartiallySignedTransaction := range updatedPartiallySignedTransactions {
		...
		isFullySigned, err := libkatwallet.IsTransactionFullySigned(updatedPartiallySignedTransaction)
		if err != nil {
			return err
		}
		if !isFullySigned {
