...
func FormatKas(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SompiPerKatkoin)
	}
	return res
}
...
func KasToSompi(amount string) (uint64, error) {
	...
	if constants.SompiPerKatkoin%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerKatkoin is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerKatkoin))
	decimalStr := ""
