...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("katd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("katd datadir: %s", dataDir)

	katdCmd, err := common.StartCmd("katd",
		"katd",
	...
	spawn("startNode-katdCmd.Wait", func() {
		err := katdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("katdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes katd closes ungracefully
				log.Errorf("katdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(katdCmd, "katdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	katminerCmd, err := common.StartCmd("MINER",
		"katminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(katminerCmd, "katminerCmd")
	return nil
}
