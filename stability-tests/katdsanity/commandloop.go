...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("katdsanity-katd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("katd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "katd-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "katd-STDERR")
