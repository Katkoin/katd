...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of katd instances
	katd1, katd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := katd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In katd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, katd2)
	}

	// Connect the two katds. This should trigger sync
	// between the two nodes
	connect(t, katd1, katd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := katd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
