...
const (
	defaultConfigFilename      = "katd.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "katd.log"
	defaultErrLogFilename      = "katd_err.log"
	...
	sampleConfigFilename    = "sample-katd.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for katd.
	DefaultAppDir = util.AppDir("katd", false)
...
// Flags defines the configuration options for katd.
//
...
// Config defines the configuration options for katd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default katd configuration
...
//
// The above results in katd functioning properly without any config settings
...
// createDefaultConfig copies the file sample-katd.conf to the given destination path,
