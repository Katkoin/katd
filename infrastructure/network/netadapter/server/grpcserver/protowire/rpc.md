...
Clients are expected to build RequestMessages and wrap them in katdMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a katdMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a katdMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network katd is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which katkoin address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether katd thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when katd is not synced. That is because when katd isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known katd addresses in the
...
currently connected to this katd.
...
| isOutbound | [bool](#bool) |  | Whether this katd initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this katd |
...
AddPeerRequestMessage adds a peer to katd&#39;s outgoing connection list.
This will, in most cases, result in katd connecting to said peer.
...
parent chain from some startHash to this katd&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this katd.
...
ShutDownRequestMessage shuts down this katd.
...
This call is only available when this katd was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given katd addresses
